...
public class PortariaEjb extends AbstractEjb implements Serializable {
	 ...
	 * Exception{ try { Portaria portaria = new Portaria();
	 * portaria.setAuditoria(aux);
	 * portaria.setIdAuditoria(portaria.getAuditoria().getId());
	 * portaria.setUnidadeGestoraPortarias( new
	 * ArrayList<UnidadeGestoraPortaria>()); for(UnidadeGestoraAuditoria x:
	 ...
	 * portaria.getUnidadeGestoraPortarias().add(unidadeGP); } return portaria;
	...
	public String ultimoNumeroPortaria(String anop) throws Exception {
		try {
			...
			// Portaria resultSql = executaSqlNativo(sql, Portaria.class,
			// entityManager).get(0);
			/*
			 * String sql =
			 * "select COALESCE(max(cast(numero_portaria as integer))+1,0) as numero_portaria from scsisaudit.portaria where ano_portaria='"
			 * + anop + "'"; String sql =
			 * "id,  id_auditoria,  processo_numero,  processo_ano,  data_assinatura,  numero_publica_doe,  data_publica_doe, "
			 * +
			 ...
			 * +
			 * "numero_portaria,  ano_portaria,  data_assinatua_portaria,  id_unidade_fiscalizadora from scsisaudit.portaria where ano_portaria='"
			 * + anop + "'";
			 */
			// List<Portaria> listaPortaria = executaSqlNativo(sql,
			// Portaria.class, entityManager);
	 ...
	 * { try { //String sql =
	 * "select * from scsisaudit.portaria where ano_portaria = '" + anop +
	 * "' order by numero_portaria desc limit 1"; String sql =
	 * "select max(cast(numero_portaria as integer))+1 as numero_portaria from scsisaudit.portaria where ano_portaria = '"
	 * + anop + "' "; List<Portaria> listaPortaria = executaSqlNativo(sql,
	 * Portaria.class, entityManager); return listaPortaria;
	 * 
	 * } catch (RuntimeException re) { re.printStackTrace(); throw new
	 * Exception(" Erro" + re.getMessage()); } catch (Exception e) {
	 * e.printStackTrace(); throw new Exception(" Erro" + e.getMessage()); } }
	 */
